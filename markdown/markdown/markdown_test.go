...
`
	...
	t.Run("heading anchor link ignores markup", func(t *testing.T) {
		...
		want := `<h2 id="a-c"><a name="a-c" class="anchor" href="#a-c" rel="nofollow" aria-hidden="true"></a><a href="B" target="_blank" rel="nofollow noreferrer noopener">A</a> C</h2>` + "\n"
		if string(doc.HTML) != want {
			t.Errorf("\ngot:  %s\nwant: %s", string(doc.HTML), want)
		}
	})
	...
	t.Run("heading consisting only of link uses link URL", func(t *testing.T) {
		...
		want := `<h2 id="a"><a name="a" aria-hidden="true"></a><a href="B" target="_blank" rel="nofollow noreferrer noopener">A</a></h2>` + "\n"
		if string(doc.HTML) != want {
			t.Errorf("\ngot:  %s\nwant: %s", string(doc.HTML), want)
		}
	})
