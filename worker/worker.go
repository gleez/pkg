// Package worker provides a workerpool. It also can expand and shrink dynamically.
//
...
func New(workers int, jobQueue ...int) *WorkerPool {
	...
	for i := 0; i < workers; i++ {
		initWorker(i+1, pool.pool, 0, nil, pool.quit, &pool.wg)
	}
...
func (pool *WorkerPool) Expand(n int, timeout time.Duration, quit <-chan struct{}) bool {
	...
	for i := 0; i < n; i++ {
		initWorker(i+1, pool.pool, timeout, quit, pool.quit, &pool.wg)
	}
	return true
}
